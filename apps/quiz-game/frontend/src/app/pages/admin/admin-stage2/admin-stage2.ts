    ...
    if (this.storage.code === "") {
      ...
      const subs = storage.messageObservable.subscribe(async (mes) => {
        if (mes.type == "song_next") {
          ...
          if (this.prev_scenario != -1)
            this.scenario = this.prev_scenario; //Bio sam ucitan kad je crklo
          else
          ...
          this.prev_scenario = this.scenario
          if (mes.team !== "null") {
            ...
            if(this.prev_scenario != 3){
              try{
                await firstValueFrom(odgovor_service.savePrevScenario(this.prev_scenario));
              }catch(err){}
            }
          }
