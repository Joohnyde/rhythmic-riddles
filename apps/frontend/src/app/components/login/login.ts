...
import { WelcomeMessage } from '../../entities/messages/default.messages';
@Component({
  selector: 'comp-login',
  imports: [FormsModule],
...
export class Login implements OnInit {
  ...
  private createServiceWithParam(code: string): WebSocketService {
    const childInjector = createEnvironmentInjector(
      [{ provide: 'room_code', useValue: code }, WebSocketService],
  ...
  async login() {
    ...
    try {
      const fwf = (await firstValueFrom(this.storage.messageObservable)) as WelcomeMessage;
      if (fwf.type == 'welcome') {
