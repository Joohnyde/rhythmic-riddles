...
class WebSocketHandlerTest {
    ...
    class WebSocketHandlerIteration7ReconnectTest {
          @Test
          void reconnectAfterInterruptResolutionSendsRecoveredPlaybackWelcomeOnlyToConnectingSocket() throws Exception {
            ...
            final WebSocketHandler handler = new WebSocketHandler(registry, gameService, interruptService);
            final WebSocketSession session = session("ws://localhost/ws/0AKKU", "AKKU", 0, true);
            final HashMap<String, Object> snapshot =
                new HashMap<>(Map.of("type", "welcome", "stage", "songs", "seek", 8.0, "remaining", 12.0));
            ...
            final Map<?, ?> json = new ObjectMapper().readValue(message.getValue().getPayload(), HashMap.class);
          ...
          @Test
          void reconnectDuringTechnicalPauseSendsErrorRecoveryWelcomeOnlyToConnectingSocket() throws Exception {
            final SessionRegistry registry = mock(SessionRegistry.class);
            final GameService gameService = mock(GameService.class);
            final InterruptService interruptService = mock(InterruptService.class);
            final WebSocketHandler handler = new WebSocketHandler(registry, gameService, interruptService);
            ...
            final HashMap<String, Object> snapshot =
                new HashMap<>(Map.of("type", "welcome", "stage", "songs", "error", true, "seek", 3.0, "remaining", 20.0));
    ...
    class WebSocketHandlerLifecycleTest {
          ...
          void afterConnectionEstablishedPushesFreshRecoverySnapshotToAcceptedSocket() throws Exception {
            ...
            verify(gameService).contextFetch("AKKU");
            verify(session).sendMessage(org.mockito.ArgumentMatchers.argThat(this::isWelcomeSongsPayload));
          }
          ...
          @Test
          void abnormalDisconnectDuringSongStageTriggersSystemInterruptRecovery() throws DerivedException {
          ...
          void normalAngularNavigationCloseDoesNotTriggerRecoveryInterrupt() throws DerivedException {
            ...
            verify(interruptService, never()).interrupt(anyString(), org.mockito.ArgumentMatchers.isNull());
          }
          ...
          void derivedInterruptFailureIsSwallowedSoSocketCloseCleanupStillCompletes() throws Exception {
            ...
            when(sessionRegistry.removeSession(session)).thenReturn(true);
            when(gameService.getStage("AKKU")).thenReturn(2);
            doThrow(new InvalidArgumentException("boom"))
    ...
    class WebSocketHandlerNonStage2LifecycleTest {
            ...
            class ConnectionClosed {
                ...
                @Test
                void nonStageTwoAbnormalDisconnectRemovesSessionButDoesNotTriggerSystemInterrupt() throws DerivedException {
                    final WebSocketSession session = attributedSession("ROOM", 0);
                    when(sessionRegistry.removeSession(session)).thenReturn(true);
                ...
                @Test
                void stageTwoAbnormalDisconnectSwallowsDerivedExceptionFromInterruptService() throws Exception {
                    final WebSocketSession session = attributedSession("ROOM", 1);
                    when(sessionRegistry.removeSession(session)).thenReturn(true);
                    when(gameService.getStage("ROOM")).thenReturn(2);
    ...
    class WebSocketHandlerRaceStressTest {
            ...
            @Test
            void repeatedStaleCloseDuringReplacementReconnectKeepsFreshAdminAndNoTechnicalPauseOutsideStageTwo() throws Exception {
                for (int i = 0; i < 30; i++) {
                    ...
                    final var closeFuture = executor.submit(() -> {
                        ready.countDown();
                        await(start);
                        handler.afterConnectionClosed(oldAdmin, CloseStatus.GOING_AWAY);
                        return null;
                    });
                    final var reconnectFuture = executor.submit(() -> {
                        ready.countDown();
                        await(start);
            ...
            void repeatedDuplicateConcurrentConnectsSendExactlyOneWelcome() throws Exception {
                for (int i = 0; i < 30; i++) {
                    ...
                    assertTrue(registry.getAdminSession("ROOM") == first || registry.getAdminSession("ROOM") == second);
                }
            }
            ...
            private static WebSocketSession session(final String roomCode, final int socketPosition, final boolean open) {
                final WebSocketSession session = mock(WebSocketSession.class);
                final Map<String, Object> attributes = new HashMap<>();
                attributes.put("ROOM_CODE", roomCode);
                attributes.put("SOCKET_POSITION", socketPosition);
    ...
    class WebSocketHandlerRecoveryRaceContractTest {
            ...
            @Test
            void staleCloseAfterReconnectDoesNotTriggerTechnicalInterruptAndReplacementStillOwnsRecovery() throws Exception {
                final WebSocketSession oldAdmin = session("ROOM", 0, true);
                ...
                org.junit.jupiter.api.Assertions.assertSame(newAdmin, sessionRegistry.getAdminSession("ROOM"));
            }

            @Test
            void reconnectingAdminGetsRecoveryFrameWithoutSendingAnythingToExistingTvOrOtherRoom() throws Exception {
    ...
    class WebSocketHandlerSerializationContractTest {
            ...
            private static WebSocketSession connectSession(final String roomCode, final int socketPosition) {
                final WebSocketSession session = mock(WebSocketSession.class);
    ...
    class WebSocketHandlerStormOrderingAndAudienceTest {
            ...
            class ConnectAudienceAndOrdering {
                ...
                @Test
                void sequentialAdminAndTvConnectionsEachReceiveOwnSingleWelcomeWithoutCrossSend() throws Exception {
                    final WebSocketSession admin = connectableSession("ROOM", 0);
            ...
            class DisconnectStorms {
                ...
                @Test
                void stageTwoAbnormalCloseStillUsesInterruptOnceEvenIfInterruptServiceThrows() throws DerivedException {
                    final WebSocketSession admin = attributedSession("ROOM", 0);
                    when(sessionRegistry.removeSession(admin)).thenReturn(true);
                    when(gameService.getStage("ROOM")).thenReturn(2);
            ...
            private static WebSocketSession connectableSession(final String room, final int socketPosition) {
