...
class WebSocketEventAndContractTest {
    ...
    class StageTwoBroadcastJsonContractSuiteTest {
            ...
            private static ScheduleEntity schedule(final double snippetDuration, final double answerDuration) {
    ...
    class StageTwoTransitionConcurrencyTest {
            ...
            void replayAndRevealCollisionBroadcastsOneValidFramePerAction() throws Exception {
                ...
                doAnswer(invocation -> {
                    ...
                    return null;
                }).when(broadcastGateway).broadcast(eq("ROOM"), anyString());

                final Future<?> replay = executor.submit(() -> {
                    try {
                        service.replaySong(schedule.getId(), "ROOM");
                    } catch (Exception ex) {
                ...
                final Future<?> reveal = executor.submit(() -> {
                    try {
                        service.revealAnswer(schedule.getId(), "ROOM");
                    } catch (Exception ex) {
                        throw new RuntimeException(ex);
            ...
            void concurrentRevealAndProgressProduceTwoIndividuallyValidContracts() throws Exception {
                ...
                final Future<?> progress = executor.submit(() -> {
                    try {
                        service.progress("ROOM");
                    } catch (Exception ex) {
                        throw new RuntimeException(ex);
                ...
                assertTrue(frames.stream().anyMatch(frame -> "song_reveal".equals(frame.get("type")) && frame.size() == 1));
    ...
    class StageTwoTransitionRaceStressRepeatedTest {
            ...
            void replayAndRevealCollisionRemainsContractValidAcrossManyRuns() throws Exception {
                for (int i = 0; i < 25; i++) {
                    ...
                    doAnswer(invocation -> {
                        synchronized (frames) {
                            frames.add(mapper.readValue(invocation.getArgument(1, String.class), HashMap.class));
                        }
                        return null;
                    }).when(broadcastGateway).broadcast(eq("ROOM"), anyString());

                    final var replay = executor.submit(() -> {
                        ready.countDown();
                        await(start);
                        service.replaySong(schedule.getId(), "ROOM");
                        return null;
                    });
                    final var reveal = executor.submit(() -> {
                        ready.countDown();
                        await(start);
                        service.revealAnswer(schedule.getId(), "ROOM");
                        return null;
                    ...
                    assertEquals(2, frames.size());
                    assertTrue(frames.stream().anyMatch(f -> "song_repeat".equals(f.get("type")) && f.containsKey("remaining") && f.size() == 2));
                    assertTrue(frames.stream().anyMatch(f -> "song_reveal".equals(f.get("type")) && f.size() == 1));
                    frames.clear();
                    reset(broadcastGateway, scheduleRepository, interruptRepository, gameService, presenceGateway);
                }
            }
            ...
            void revealAndProgressCollisionRemainsContractValidAcrossManyRuns() throws Exception {
                for (int i = 0; i < 25; i++) {
                    ...
                    final var progress = executor.submit(() -> {
                        ready.countDown();
                        await(start);
                        service.progress("ROOM");
                        return null;
    ...
    class WebSocketContractEventsTest {
          ...
          class CategoryAndScheduleEventContracts {
            ...
            void setUp() {
              ...
              ReflectionTestUtils.setField(scheduleServiceImpl, "interruptRepository", interruptRepository);
          ...
          class InterruptEventContracts {
            ...
            void answerBroadcastsResolvedGuessContractFrame() throws Exception {
              ...
              assertTrue(payload.getValue().contains("\"teamId\":\"" + answer.getTeamId().getId() + "\""));
              assertTrue(payload.getValue().contains("\"scheduleId\":\"" + answer.getScheduleId().getId() + "\""));
          ...
          private static InterruptEntity answerInterrupt(final GameEntity game, final double snippetDuration) {
            final TeamEntity team = team(game, "Blue");
    ...
    class WebSocketJsonContractLockStageTwoTest {
            ...
            class BroadcastFrames {
                ...
                void pauseFrameLocksRequiredFieldsAndNoLeaks() throws Exception {
                    ...
                    when(interruptRepository.findInterrupts(startedAt, schedule.getId())).thenReturn(new ArrayList<>());
            ...
            class RecoveryWelcomeFrames {
                ...
                @Test
                void endedNotRevealedWelcomeLocksRevealedFalseAndDropsPlaybackProgressFields() throws Exception {
                    final UUID gameId = UUID.randomUUID();
                    final GameEntity game = songsGame(gameId);
                    ...
                    when(scheduleRepository.findLastPlayed(gameId)).thenReturn(schedule);
                    when(teamService.getTeamScores("ROOM")).thenReturn(scorePayload());
                    when(interruptService.calculateSeek(schedule.getStartedAt(), schedule.getId())).thenReturn(12000L);
