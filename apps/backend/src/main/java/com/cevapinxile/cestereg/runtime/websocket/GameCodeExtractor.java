...
/**
 * @author denijal Extracts room code and client slot information from the WebSocket handshake URL.
 *     <p>Expected format: {@code /ws/{pos}{roomCode}}, where {@code pos} is a single digit
 *     representing the client type/slot (e.g., 0=admin, 1=tv). This keeps the handshake free of
 *     query parameters and ensures routing is explicit.
 *     <p>If extraction fails (missing/invalid slot or room code), the caller should treat the
 *     session as unauthenticated and close it.
 */
@Component
public class GameCodeExtractor implements HandshakeInterceptor {
   ...
   *
   * <p>Expected URL format: {@code /ws/{pos}{roomCode}}, where {@code pos} is a single digit
   * identifying the client type/slot (e.g., 0 = ADMIN, 1 = TV).
   *
   * <p>On successful validation, this method stores handshake-derived attributes on the session:
   *
   * <ul>
   *   <li>{@code ROOM_CODE} - the room identifier (without the prefix digit)
   *   <li>{@code SOCKET_POSITION} - the client slot/type (0 or 1)
   * </ul>
   *
   * These attributes are later used by the WebSocket handler to route messages and clean up
   * sessions.
   *
   * <p>The handshake is rejected ({@code false}) if the room code is unknown or the slot prefix is
   * invalid.
   *
  ...
  public boolean beforeHandshake(
    ...
    final Integer socketPosition = (int) roomCode.charAt(0) - '0';
    ...
    if (gameRepository.findByCode(roomCode).isEmpty()
        || (socketPosition != 0 && socketPosition != 1)) {
      return false;
    }
