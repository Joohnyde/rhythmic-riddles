...
class TeamScore {
  ...
  TeamScore(HashMap<UUID, TeamScore> scoreMap, TeamScoreProjection projection) {
...
public class TeamScoreCache {
  ...
  public TeamScoreCache(List<TeamScoreProjection> projections) {
    if (projections != null) {
      scoreList =
          projections.stream().map((projection) -> new TeamScore(scoreMap, projection)).toList();
    }
  }
  ...
  public void setScore(UUID teamId, UUID scheduleId, Integer score)
    ...
    final TeamScore teamScore = this.scoreMap.get(teamId);
