...
/**
 * @author denijal WebSocket entry point for the quiz runtime.
 *     <p>Handles the connection lifecycle (connect / message / close) and routes events to the
 *     correct game room using handshake-derived identifiers.
 *     <p>Client type (e.g., admin vs TV) is encoded in the WebSocket URL and extracted during the
 *     handshake. Disconnect handling is defensive because SPAs (Angular) may close sockets on
 *     navigation with a normal close code, while refresh / browser close typically yields a
 *     different close code.
 *     <p>This handler is responsible for best-effort cleanup to prevent leaking session state and
 *     to ensure the game can recover after reconnects.
 */
public class WebSocketHandler extends TextWebSocketHandler {
  ...
  public WebSocketHandler(
      SessionRegistry sessionRegistry, GameService gameService, InterruptService interruptService) {
   ...
   *
   * <p>On success, the session is stored in the {@code SessionRegistry} under a key derived from
   * the handshake. On failure (invalid handshake, unknown room, etc.), the session should be closed
   * and ignored.
   *
   ...
   *
   * <p>Close events can occur due to "normal" SPA navigation, refreshes, browser closes, or network
   * drops. Cleanup is guarded to avoid accidental removal of a newer session in case of reconnects.
   *
  ...
  public void afterConnectionClosed(final WebSocketSession session, final CloseStatus status) {
    ...
    if (session == null
        ...
        || !session.getAttributes().containsKey("ROOM_CODE")) {
      return;
    }
    final String roomCode = session.getAttributes().get("ROOM_CODE").toString();
    ...
    if (sessionRegistry.removeSession(session)
        && this.gameService.getStage(roomCode) == 2
        && status.getCode() != 1000) {
      try {
