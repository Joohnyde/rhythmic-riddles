...
""")
            ...
            @Content(
                ...
                examples =
                    @ExampleObject(
                        value =
                            "{\"error\":\"E002 - Malformed argument\","
                                + "\"message\":\"stage_id must be between 0 and 3 and must differ from the current stage.\"}"))),
    @ApiResponse(
        responseCode = "409",
                    ...
                    @ExampleObject(
                        value =
                            "{\"error\":\"E003 - Wrong game-state\","
                                + "\"message\":\"Illegal stage transition: only 0->1, 1->2, 2->1, 2->3 are allowed.\"}"))),
    @ApiResponse(
        responseCode = "503",
  ...
  public ResponseEntity<?> changeState(
      @RoomCodePath @PathVariable String roomCode, @RequestBody StageIdRequest stageId) {
    try {
