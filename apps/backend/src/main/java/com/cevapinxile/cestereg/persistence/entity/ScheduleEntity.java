...
@NamedQueries({
  ...
  @NamedQuery(
      name = "ScheduleEntity.findByOrdinalNumber",
      query = "SELECT s FROM ScheduleEntity s WHERE s.ordinalNumber = :ordinalNumber"),
  @NamedQuery(
      name = "ScheduleEntity.findByStartedAt",
      query = "SELECT s FROM ScheduleEntity s WHERE s.startedAt = :startedAt"),
  @NamedQuery(
      name = "ScheduleEntity.findByRevealedAt",
      query = "SELECT s FROM ScheduleEntity s WHERE s.revealedAt = :revealedAt"),
  @NamedQuery(
      name = "ScheduleEntity.findLastPlayed",
      query =
          """
              SELECT s
...
public class ScheduleEntity implements Serializable {
  ...
  public ScheduleEntity(
      E2eGameFixtureRequest.Schedule scheduleFixture,
      CategoryEntity newCategory,
      TrackEntity newTrack) {
    this.id = scheduleFixture.id();
    this.ordinalNumber = scheduleFixture.ordinalNumber();
    this.startedAt = scheduleFixture.startedAt();
    this.revealedAt = scheduleFixture.revealedAt();
    this.categoryId = newCategory;
    this.trackId = newTrack;
  }
